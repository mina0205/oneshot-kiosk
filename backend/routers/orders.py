...
from utils import find_menu, build_cart_response, _coupon_index, _promo_indexfrom store import carts, orders
import store
...
def create_order(session_id: str, req: OrderRequest):
    ...
    coupon_discount = 0
    if req.couponId:
        coupon = _coupon_index.get(req.couponId)
    ...
    for item in cart_response["items"]:
        menu_promos = _promo_index.get(item["menuId"], [])
    ...
    if session_id not in store.order_index:
    store.order_index[session_id] = {}
store.order_index[session_id][order["orderId"]] = order
...
def reorder(session_id: str, orderId: str):
    ...
    target_order = store.order_index.get(session_id, {}).get(orderId)
