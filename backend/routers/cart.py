...
def add_to_cart(session_id: str, item: CartItemRequest):
    ...
    # 동일 메뉴+옵션 있으면 수량만 합산
    for existing_id, existing in carts[session_id].items():
        if (existing["menuId"] == item.menuId
                and existing["isSet"] == item.isSet
                and existing["selectedSide"] == item.selectedSide
                and existing["selectedDrink"] == item.selectedDrink
                and existing["drinkSize"] == item.drinkSize
                and existing["toppings"] == (item.toppings or [])):
